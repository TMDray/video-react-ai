...
@dataclass
class VideoResult:
    """Represents a single video search result with all metadata."""
    id: str
    title: str
    description: Optional[str]
    url: str  # page URL
...
class StockVideoManager:
    """Manages video search via Pexels Video API (same API key as images)."""

    def __init__(self):
        self.api_key = os.getenv("PEXELS_API_KEY")

    def _curl_get(self, url: str, params: Optional[Dict] = None) -> Optional[Dict]:
        """Make a GET request with pycurl and return parsed JSON."""
        if not self.api_key:
            return None
        buffer = io.BytesIO()
        c = pycurl.Curl()
        ...
        c.setopt(pycurl.URL, full_url)
        c.setopt(pycurl.WRITEDATA, buffer)
        c.setopt(pycurl.HTTPHEADER, [f"Authorization: {self.api_key}"])
        c.setopt(pycurl.FOLLOWLOCATION, True)
        c.perform()
        status = c.getinfo(pycurl.HTTP_CODE)
        c.close()
        if status != 200:
            logger.error(f"Pexels Video API error: HTTP {status}")
            return None
        return json.loads(buffer.getvalue().decode("utf-8"))

    def _parse_video(self, v: Dict) -> VideoResult:
        """Parse a Pexels video JSON object into a VideoResult."""
        files = []
        ...
        if pics:
            best_pic = pics[0].get("picture", "")
        return VideoResult(
            id=f"pexels_video_{v['id']}",
            title=v.get("url", "").split("/")[-1].replace("-", " ").title() if v.get("url") else f"Video {v['id']}",
            description=None,
            url=v.get("url", ""),
            thumbnail=best_pic,
            width=v.get("width", 0),
            ...
            fps=files[0].get("fps") if files else None,
    ...
    async def search(self, query: str, per_page: int = 10, page: int = 1,
        ...
        if not self.api_key:
            return {"error": "PEXELS_API_KEY not set. Video search requires a Pexels API key."}
        params: Dict[str, Any] = {"query": query, "per_page": min(per_page, 50), "page": page}
        if min_duration:
        ...
        data = self._curl_get("https://api.pexels.com/videos/search", params)
        if data is None:
            return {"error": "Failed to search Pexels Video API"}
        ...
        return {
            ...
            "results": [asdict(v) for v in videos],
        }

    async def get_details(self, video_id: str) -> Dict[str, Any]:
        """Get details for a specific Pexels video."""
        if not self.api_key:
            return {"error": "PEXELS_API_KEY not set."}
        pexels_id = video_id.replace("pexels_video_", "")
        data = self._curl_get(f"https://api.pexels.com/videos/videos/{pexels_id}")
        if data is None:
            return {"error": f"Video {video_id} not found"}
        return asdict(self._parse_video(data))

    async def download(self, video_id: str, quality: str = "hd",
                       output_path: Optional[str] = None) -> Dict[str, Any]:
        """Download a video file."""
        details = await self.get_details(video_id)
        if "error" in details:
            return details
        files = details.get("video_files", [])
        if not files:
            return {"error": "No video files available"}
        # Pick best matching quality
        target = None
        for f in files:
        ...
        if not target:
            ...
            target = max(files, key=lambda f: (f.get("width", 0) or 0))
        download_url = target.get("link")
        if not download_url:
        ...
        try:
            ...
            c.perform()
            status = c.getinfo(pycurl.HTTP_CODE)
            content_type = c.getinfo(pycurl.CONTENT_TYPE) or "video/mp4"
            c.close()
            if status != 200:
            ...
            if output_path:
                ...
                return {
                    "success": True,
                    "message": f"Video downloaded to {abs_path}",
                    "path": abs_path,
                    "size_mb": round(len(video_data) / 1024 / 1024, 2),
                    "quality": target.get("quality"),
                    "resolution": f"{target.get('width')}x{target.get('height')}",
                }
            else:
                return {
                    "success": True,
                    "message": "Video too large for base64. Please provide output_path.",
                    "size_mb": round(len(video_data) / 1024 / 1024, 2),
                }
        except pycurl.error as e:
...
class StockyServer:
    """Main MCP server for stock image and video searching."""

    def __init__(self):
        self.mcp = FastMCP("stocky")
        self.manager = StockImageManager()
        self.video_manager = StockVideoManager()
        self._setup_tools()
    ...
    def _setup_tools(self):
        ...
        @self.mcp.tool("search_stock_videos")
        async def search_stock_videos(
            ...
            """
            Search for royalty-free stock videos on Pexels.

            Args:
                query: Search query string (e.g. "office meeting", "nature aerial")
            ...
            Returns:
                Search results with video metadata and download links
            """
            return await self.video_manager.search(
                query, per_page, page, min_duration, max_duration, orientation
            )

        @self.mcp.tool("get_video_details")
        async def get_video_details(video_id: str) -> Dict[str, Any]:
            """
            Get detailed information about a specific stock video.

            Args:
                video_id: Video ID in format pexels_video_123456

            Returns:
                Video details including all available quality variants
            """
            return await self.video_manager.get_details(video_id)

        @self.mcp.tool("download_video")
        async def download_video(
            ...
            Args:
                video_id: Video ID in format pexels_video_123456
                quality: Quality variant: "sd", "hd", or "uhd" (default: "hd")
                output_path: Absolute path to save the video (e.g. /path/to/public/videos/bg.mp4).
                    Use absolute paths to ensure correct location.

            Returns:
                Path to downloaded file and metadata
            """
            return await self.video_manager.download(video_id, quality, output_path)
